...
이 레포지토리를 작업하는 **모든 AI 어시스턴트는 문서 구조의 통일성과 질의응답 및 작업 이력의 완벽한 연속성을 위해 아래 7대 협업 규칙을 철저히 준수**해야 합니다.

---

## 📜 AI 필수 준수 7대 수칙 (Mandatory AI Rules)

### 1. 🔗 상호 쌍방향 마크다운 십자 링크 필수 (Mandatory Bidirectional Cross-Linking Rule)
...
### 2. 📐 통일된 표준 문서 구조 준수 (Unified Document Structure Rule)
...
### 3. 📝 사용자 질의응답 및 해결 답변의 100% 상세 문서화 (Rich Detail Rule)
* 사용자와 주고받은 모든 질의응답(Q&A), 에러 원인 분석, 펌웨어 업그레이드 절차, 파라미터 의미, 명령어를 절대 함축하거나 생략하지 말고 **풍부하고 정밀한 기술적 디테일**로 해당 문서의 Q&A 섹션에 수록한다.

### 4. 🗂️ Hardware 및 Software 다중 관련 카테고리 중복 수록 허용 (Cross-Referencing Rule)
...
### 5. 📢 루트 `README.md` 최신 업데이트 섹션 필히 반영 (Root README Mandate)
...
### 6. 💻 개발 환경 및 AI 이력 트래킹 리포트 저장 (Environment Audit Rule)
...
### 7. 🔄 다른 AI가 즉시 작업을 이어받을 수 있는 완벽한 연속성 보장 (Seamless Continuation)
